...
def find_location(mini_map):
  large_map = cv2.imread('./worldMap/map5.png')
  ...
  cv2.circle(large_map, player_position, 4, (255, 0, 0) , 2)
  cv2.imwrite('./worldMap/result' + str(int(time.time())) + '.png', large_map)

  return player_position

def navigate_to_location(location: str, mini_map):
  position = locations[location]
  current_position = find_location(mini_map)
